...
| Integrity layer + injected 4% Test-station drop | Day 2-3 | Day 3 | Core deliverable, took the most iteration. Settled on "missing scan" meaning *reached a later station but skipped this one* rather than any incomplete unit, so units still in flight aren't false-flagged. First end-to-end run then exposed a real bug: Pack was subscribed directly to Test's scan-event topic, so a dropped Test scan also silently kept the unit out of Pack -- collapsing the exact gap the integrity layer needed to see. Fixed by splitting Test's output into an always-published "advance" signal (physical hand-off) and the scan-data event (which still drops ~4%). |
